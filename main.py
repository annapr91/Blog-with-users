...
from forms import CreatePostForm, RegisterForm, Loging,CommentForm
...
def show_post(post_id):
    ...
    coment=db.session.execute(db.select(Comment))
    coments=coment.scalar()
    print(requested_post)

    if form.validate_on_submit():

        if not current_user.is_authenticated:
            flash('You are not loging to leave a comment')
    ...
    return render_template("post.html", post=requested_post, form=form, coments=coments)
